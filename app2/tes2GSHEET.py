...
def get_active_registration1(name: str):
    conn = _get_conn()
    # pesquisar por nome; se ja existir entao...
    sql = ''
    reg_data_exist = conn.query(sql=sql)
    if reg_data_exist:
        return True
    ...
    active_reg = reg_df[(reg_df["name"] == name) & (reg_df["is_active"] == 1)]
    if active_reg.empty:
        return None



    return active_reg.iloc[0].to_dict()
